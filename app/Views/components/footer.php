      ...
      <footer class="footer">
          <div class="footer-body">
              ...
              <div class="right-panel">
                  ©2026 Made with
                  <span class="">
                      <svg class="icon-15" width="15" viewBox="0 0 24 24" fill="none" xmlns="">
    ...
    <a class="btn btn-fixed-end btn-warning btn-icon btn-setting" data-bs-toggle="offcanvas" data-bs-target="#offcanvasExample" role="button" aria-controls="offcanvasExample">
      <svg width="24" viewBox="0 0 24 24" class="animated-rotate icon-24" fill="none" xmlns="">
        ...
        <div class="row">
          <div class="col-lg-12">
            ...
            <button class="btn btn-transparent p-0 border-0" data-value="theme-color-default" data-info="#079aa2" data-setting="color-mode1" data-name="color" data-bs-toggle="tooltip" data-bs-placement="top" title="" data-bs-original-title="Default">
              <svg class="icon-18" width="18"  viewBox="0 0 24 24" fill="none" xmlns="">
            ...
            <div class="grid-cols-5 mb-4 d-grid gap-x-2">
              ...
              <svg  class="customizer-btn icon-32" xmlns="" viewBox="0 0 24 24" width="32" > <circle cx="12" cy="12" r="10" fill="#00C3F9" /> <path d="M2,12 a1,1 1 1,0 20,0" fill="#573BFF" /></svg>
              </div>
              ...
              <svg  class="customizer-btn icon-32" xmlns="" viewBox="0 0 24 24" width="32" > <circle cx="12" cy="12" r="10" fill="#91969E" /> <path d="M2,12 a1,1 1 1,0 20,0" fill="#FD8D00" /></svg>
              </div>
              ...
              <svg  class="customizer-btn icon-32" xmlns="" viewBox="0 0 24 24" width="32" > <circle cx="12" cy="12" r="10" fill="#DB5363" /> <path d="M2,12 a1,1 1 1,0 20,0" fill="#366AF0" /></svg>
              </div>
              ...
              <svg  class="customizer-btn icon-32" xmlns="" viewBox="0 0 24 24" width="32" > <circle cx="12" cy="12" r="10" fill="#EA6A12" /> <path d="M2,12 a1,1 1 1,0 20,0" fill="#6410F1" /></svg>
              </div>
              ...
              <svg  class="customizer-btn icon-32" xmlns="" viewBox="0 0 24 24" width="32" > <circle cx="12" cy="12" r="10" fill="#E586B3" /> <path d="M2,12 a1,1 1 1,0 20,0" fill="#25C799" /></svg>
              </div>
...
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
 <style>
  /* Custom SweetAlert Overrides */
.swal2-popup.rounded-2xl {
    border-radius: 1rem !important;
}

.swal2-confirm.rounded-lg {
...
<script>
/**
 * GLOBAL SECURITY & SESSION INTERCEPTOR
 * Unified Version: Prevents false positives when DevTools are closed
 * and uses JWT as the "Source of Truth."
 */
...
// 1. GRACE PERIOD
// Mutes security alerts for the first 3 seconds to allow session stabilization
setTimeout(() => { 
    pageReady = true; 
    console.log("Security Interceptor: Active");
}, 3000);
...
// 3. JWT VALIDATOR
// Checks the 'exp' claim in your token locally
function isActuallyExpired() {
    ...
    try {
        ...
        // Return true if current time has passed the expiry time
...
/**
 * 4. MASTER JQUERY AJAX INTERCEPTOR
 * Handles DataTables and all $.ajax calls
 */
$(document).ajaxError(function(event, jqXHR, ajaxSettings, thrownError) {
    ...
    // B. AUTHENTICATED BYPASS
    // If the server sent our "Hall Pass" header, ignore errors
    const authStatus = jqXHR.getResponseHeader('X-Auth-Status');
    if (authStatus === 'Authenticated') return;

    // C. HANDLE 401 (Unauthorized / Session Expired)
    if (jqXHR.status === 401) {
        // Double check: Only show popup if the JWT clock has actually run out
        if (isActuallyExpired()) {
            handleExpiry();
        } else {
            console.warn("Auth: Server reported 401, but JWT is still valid. Ignoring glitch.");
        }
    }

    // D. HANDLE 403 (Forbidden / Permission Denied)
    if (jqXHR.status === 403) {
        Swal.fire({
            icon: 'error',
            title: 'Access Denied',
...
window.fetch = async (...args) => {
    try {
        ...
        if (response.headers.get("X-Auth-Status") === "Authenticated") {
            return response;
        }

        if (response.status === 401 && !isPageUnloading && pageReady) {
            if (isActuallyExpired()) {
                handleExpiry();
                return new Promise(() => {}); // Kill the promise chain
            }
        }

        return response;
    } catch (err) {
        if (isPageUnloading) return new Promise(() => {});
        throw err;
    }
};

/**
 * 6. SESSION EXPIRY POPUP (SweetAlert2)
 */
function handleExpiry() {
    ...
    Swal.fire({
        title: '<span style="color: #1e293b; font-weight: 700;">Session Expired</span>',
        html: `
            <div style="color: #64748b; font-size: 0.95rem; line-height: 1.5;">
                Your security token has timed out.<br>
                Redirecting to login in <b id="swal-timer" style="color: #4f46e5;">10</b> seconds...
            </div>
        `,
        icon: 'warning',
        iconColor: '#f59e0b',
        ...
        didOpen: () => {
            const timerBridge = Swal.getHtmlContainer().querySelector('#swal-timer');
            timerInterval = setInterval(() => {
                const timeLeft = Swal.getTimerLeft();
                if (timerBridge) {
                    timerBridge.textContent = Math.ceil(timeLeft / 1000);
                }
            }, 100);
        },
        willClose: () => {
